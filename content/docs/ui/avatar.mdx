...
<div className="my-6 rounded-lg border border-orange-200 bg-orange-50 p-4 dark:border-orange-800 dark:bg-orange-950">
  <div className="flex">
    <div className="ml-3">
...
>(({ className, size, tooltip, ...props }, ref) => {
  const avatar = (
    <AvatarPrimitive.Root
      ref={ref}
      className={PS(avatarVariants({ size }), className)}
      {...props}
    />
  );
...
>(({ className, ...props }, ref) => (
  <AvatarPrimitive.Image
    ref={ref}
    className={PS("aspect-square h-full w-full object-cover", className)}
    {...props}
  />
));
  ...
  <Tab value="Preview">
    <PreviewContainer>
      <AvatarGroup spacing="tight">
        ...
        <Avatar tooltip={{ children: "Vercel", variant: "dark" }}>
          <AvatarImage src="https://github.com/vercel.png" alt="@vercel" />
          <AvatarFallback>VC</AvatarFallback>
        </Avatar>
...
| `tooltip`   | `string` \| `TooltipContentProps`                         | `undefined` | Tooltip content or props for the avatar |
...
| `size`      | `"xs"` \| `"sm"` \| `"md"` \| `"lg"` \| `"xl"` \| `"2xl"` | `"md"`      | Size of all avatars in the group  |
| `children`  | `React.ReactElement[]`                                    | required    | Array of Avatar components        |
| `className` | `string`                                                  | `undefined` | Additional CSS classes            |

...
```package-install
npx hexta-ui@latest add command-menu
```
...
The command menu is a modal dialog that provides quick access to commands and actions through keyboard navigation. It automatically detects your operating system and displays the appropriate keyboard shortcuts (⌘K on macOS, Ctrl+K on Windows/Linux). The component provides an interface similar to VS Code's command palette.
...
The Command Menu component is built with accessibility in mind:

- **Keyboard Navigation**: Arrow keys navigate through items, Enter selects, Escape closes
- **Screen Reader Support**: Proper ARIA labels and roles for assistive technologies
- **Focus Management**: Focus is trapped within the modal and properly restored on close
- **Global Shortcut**: Configurable keyboard shortcut (default: ⌘K) for quick access
...
- **Entry**: Scale and fade in with slight upward movement
- **Exit**: Scale and fade out with slight downward movement
- **Duration**: 200ms with easeOut timing function
